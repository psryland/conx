	...
	struct Cmd_Exec
	{
		void ShowHelp() const
		{
			std::cout <<
				...
				" Syntax: Conx -exec [-async] [-cwd working_dir] -p exe_path args ... \n"
				...
				" -async : Optional parameter that causes Conx to return immediately\n"
				"     By default, Conx will block until the process has completed.\n"
