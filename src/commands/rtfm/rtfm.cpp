...
//**********************************************
// Rtfm: Output complete markdown documentation of all conx commands.
...
namespace conx
{
	struct Cmd_Rtfm
	{
		...
		{
			std::cout <<
				"Rtfm: Output complete documentation for all conx commands\n"
				" Syntax: conx -rtfm\n"
				"\n"
		...
		int Run(pr::CmdLine const& args)
		{
			...
			std::cout << R"md(# CONX - Console EXtensions

A multi-purpose command-line utility for Windows. Each command is invoked as:

```
conx -<command> [parameters]
```

Use `conx -<command> -help` for built-in help on any command.
...
```
conx -clip "text" [-lwr] [-upr] [-fwdslash] [-bkslash] [-cstr] [-crlf|-cr|-lf]
conx -clip -paste
```
...
```
conx -dirpath <env_var_name> [-msg "Message"]
```
...
```
conx -msgbox -title "title" -body "body text" -style <style_id>
```
...
```
conx -screenshot -p <process-name> -o <output-dir> [-all] [-bitblt] [-scale N]
```
...
```
conx -send_keys "text" -p <process-name> [-w <window-name>] [-rate <keys_per_sec>]
```
...
```
conx -send_mouse x,y -b <action> -p <process-name> [-w <window-name>]
```
...
- For programmatic use, redirect stdout: `Start-Process conx.exe '-command args' -RedirectStandardOutput out.txt -Wait`
...
- conx is DPI-aware (Per-Monitor V2). All coordinates and captures use physical pixels.
)md";
			return 0;
		}
	};
